    ...
    while (SDL_PollEvent(&event)) {
        ...
        case SDL_KEYDOWN:
			...
			case SDL_SCANCODE_SPACE:
				if(app->scene.player_velocity==0) app->scene.player_velocity-=0.1f;
				break;
			case SDL_SCANCODE_F1:
				app->show_hud=app->show_hud?false:true;
				break;
			case SDL_SCANCODE_F:
...
void update_app(App* app)
{
    ...
    int result=update_camera(&(app->camera), elapsed_time,&(app->scene));
	if(result==-1){
		app->is_running=false;
	}
